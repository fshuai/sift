...
import sift.ImagePixelArray;
import sift.RefFloat;
import sift.FloatArray;
 ...
 */
public class OctaveSpace implements Serializable{
    ...
    public  void pretreatMagnitudeAndDirectionImgs() {
        ...
        magnitudes = new ImagePixelArray[smoothedImgs.length - 1];// 梯度的数组
        directions = new ImagePixelArray[smoothedImgs.length - 1];// 方向的数组
        for (int s = 1; s < (smoothedImgs.length - 1); s++) {
            magnitudes[s] = new ImagePixelArray(smoothedImgs[s].width,
                    smoothedImgs[s].height);
            directions[s] = new ImagePixelArray(smoothedImgs[s].width,
                    smoothedImgs[s].height);
    ...
    public  ArrayList<FeaturePoint> makeFeaturePoints(
        ...
        for (ScalePeak sp : localizedPeaks) {
            ArrayList<FeaturePoint> thisPointKeys = makeFeaturePoint(
                    baseScale, sp, peakRelThresh, scaleCount, octaveSigma);
    ...
    private  ArrayList<FeaturePoint> makeFeaturePoint(float imgScale,
        ...
        for (int b = 0; b < 36; ++b) {
            ...
            FeaturePoint fp = new FeaturePoint(smoothedImgs[point.level],
    ...
    private static boolean interpolateOrientation(float left, float middle,
    ...
    private static void averageBoxes(float[] boxes) {
        // ( 0.4, 0.4, 0.3, 0.4, 0.4 ))
        // 每三个做1个平均直至完成
    ...
    private static int findClosestRotationBox(float angle) {
        angle += Math.PI;
        angle /= 2.0f * Math.PI;
        angle *= 36;
        int idx = (int) angle;
    ...
    private static ArrayList<FeaturePoint> createDescriptors(
    ...
    private static void capAndNormalizeFV(FeaturePoint kp, float fvGradHicap) {
     ...
     * @return
     */
    private static ArrayList<ScalePeak> findPeaks4ThreeLayer(ImagePixelArray below,
    ...
    private static void checkMinMax(ImagePixelArray layer, float c, int x, int y,
